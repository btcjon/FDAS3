...
df = asyncio.run(fetch_and_update_positions())  # Update this line to capture the returned df

# Update positions_all_grouped value
...
df1_styled = df1.style.apply(color_conditions, axis=1)
df1_styled = df1_styled.applymap(color_negative_red, subset=['uProfit', 'swap'])
